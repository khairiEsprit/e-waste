...
public class TraitementController implements Initializable {
    ...
    public void loadTraitementByDemande(int idDemande) {
        try {
            ...
            Modals.displayError("Erreur", "Impossible de charger les traitements.");
        }
    }
...
//        openAddTraitementForm(selectedTraitement.getIdDemande());
        try {
            ...
            Modals.displayError("Erreur", "Impossible d'ouvrir le formulaire.");
        }
    ...
    private void openEditTraitementForm(Traitement traitement) {
      ...
      System.out.println(selectedTraitement);
        if (selectedTraitement == null) {
            Modals.displayError("Erreur", "Veuillez sélectionner un traitement à modifier.");
            return;
        }
        ...
        try {
            ...
            Modals.displayError("Erreur", "Impossible d'ouvrir la modification.");
        }
    }
        ...
        confirmationAlert.showAndWait().ifPresent(response -> {
            if (response == buttonTypeOK) {
                try {
                    ...
                    Modals.displaySuccess("Succès", "Traitement supprimé avec succès.");
                } catch (SQLException e) {
                    Modals.displayError("Erreur", "Impossible de supprimer le traitement.");
                }
            }
        });
